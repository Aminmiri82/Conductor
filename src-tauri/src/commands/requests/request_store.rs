...
#[tauri::command]
pub fn get_request(request_id: String, state: State<'_, AppState>) -> AppResult<RequestDetail> {
        ...
        let mut request: RequestDetail = connection.query_row(
            ...
            |row| {
                ...
                Ok(RequestDetailRaw {
                    ...
                    auth_json,
                    body_json,
                    pre_request_script_json,
                    test_script_json,
                    updated_at: row.get(12)?,
                })
            },
        )?.into_request(&secrets)?;
        request.inherited_auth =
            request_auth::inherited_auth_for_request(connection, &secrets, &request)?;
        request.effective_auth =
...
pub fn duplicate_request(
    ...
    state: State<'_, AppState>,
) -> AppResult<DuplicateRequestResult> {
    let now = Utc::now().to_rfc3339();
...
#[tauri::command]
pub fn delete_request(request_id: String, state: State<'_, AppState>) -> AppResult<()> {
    state.database.with_connection(|connection| {
...
#[tauri::command]
pub fn save_request(request: RequestDetail, state: State<'_, AppState>) -> AppResult<()> {
    let now = Utc::now().to_rfc3339();
    let secrets = state.database.secrets().clone();
    state.database.with_connection(|connection| {
        ...
        let auth_json = match request.auth.as_ref() {
            Some(auth) => Some(request_auth::encode_auth_json(&secrets, auth)?),
            None => None,
        };
...
impl RequestDetailRaw {
    fn into_request(self, secrets: &Secrets) -> Result<RequestDetail, StorageError> {
        let auth = match self.auth_json.as_deref() {
            Some(value) => request_auth::decode_auth_json(secrets, value)?,
            None => None,
        };
        Ok(RequestDetail {
            id: self.id,
            ...
            method: self.method,
            url: self.url,
            headers: parse_json(&self.headers_json),
            query: parse_json(&self.query_json),
            path_params: parse_json(&self.path_params_json),
            auth,
            inherited_auth: None,
            effective_auth: None,
            body: self.body_json.as_deref().and_then(parse_json_optional),
            pre_request_script: self
                .pre_request_script_json
                .as_deref()
                .and_then(parse_json_optional),
            test_script: self.test_script_json.as_deref().and_then(parse_json_optional),
            updated_at: self.updated_at,
        })
    }
}

fn parse_json<T>(value: &str) -> T
where
